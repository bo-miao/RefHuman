...
def crop(image, target, region):
    ...
    if "boxes" in target or "masks" in target:
        ...
        for field in fields:
            if field in target and field != 'masks':
    ...
    if "scribble" in target:
        max_size = torch.as_tensor([w, h], dtype=torch.float32).view(1, 1, 2)
        scribble = target["scribble"]
        cropped_scribble = scribble - torch.as_tensor([j, i]).view(1, 1, 2)
        cropped_scribble = torch.min(cropped_scribble, max_size)
        cropped_scribble = cropped_scribble.clamp(min=0)
        target["scribble"] = cropped_scribble
...
def hflip(image, target):
    ...
    if "scribble" in target:
        target["scribble"][..., 0] = w - target["scribble"][..., 0] - 1
...
def resize(image, target, size, max_size=None):
    ...
    if "scribble" in target:
        target["scribble"] = target["scribble"] * torch.as_tensor([ratio_width, ratio_height]).view(1, 1, 2)
...
class Normalize(object):
    def __init__(self, mean, std, eval_mode=False, official_coco=False):
        self.mean = mean
        self.std = std
        self.eval_mode = eval_mode
        self.official_coco = official_coco
    ...
    def __call__(self, image, target=None):
        ...
        if "scribble" in target:  # 12,2
            target["scribble"][..., 0] = torch.clamp(target["scribble"][..., 0], min=0., max=w-1)
            target["scribble"][..., 1] = torch.clamp(target["scribble"][..., 1], min=0., max=h-1)
            target["scribble"] = target["scribble"].floor()
