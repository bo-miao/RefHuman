...
class MSDeformAttn(nn.Module):
    ...
    def forward(self, query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, input_padding_mask=None):
        ...
        # *******
        if reference_points.shape[-1] == 6 or reference_points.shape[-1] == 34 or reference_points.shape[-1] == 36 or reference_points.shape[-1] == 30:
            bs, nq, nl, np = reference_points.shape
            reference_points = reference_points.reshape(bs, nq, nl, np // 2, 2)
            reference_points = torch.transpose(reference_points, 2, 3).flatten(1, 2)  # b 100*18 4 2
        ...
        # print(sampling_locations.shape) # torch.Size([b, 100*18, [8] head, 4, [4] num, 2])
        # for amp
