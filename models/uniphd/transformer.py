...
class Transformer(nn.Module):
    ...
    def __init__(self, args, d_model=256,
        ...
        # build deformable multimodal encoder & pose-centric hierarchical decoder
        ...
        decoder_layer = DeformableTransformerDecoderLayer(d_model, dim_feedforward, dropout, activation, num_feature_levels,
                                                          nhead, dec_n_points, pose_guide=args.pose_guide_trigger,
                                                          n_query=num_queries, late_within_attn=args.late_within_attn_trigger,
                                                          within_type=args.within_type, n_kps=num_body_points)
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed):
        ...
        if self.two_stage_type in ['standard']:
            ...
            enc_outputs_class_unselected = self.enc_out_class_embed(output_memory)  # scores for selection
            topk_proposals = torch.topk(enc_outputs_class_unselected.max(-1)[0], topk, dim=1)[1]
            # estimate keypoints & center for top-k positions and used as reference points
            ...
            # retrieve top-k multimodal embeddings for later keypoint query initialization
            tgt_undetach = torch.gather(output_memory, 1, topk_proposals.unsqueeze(-1).repeat(1, 1, self.d_model))
            if self.learnable_tgt_init:
                tgt = self.tgt_embed.expand_as(tgt_undetach).unsqueeze(-2)
            else:
                tgt = tgt_undetach.detach().unsqueeze(-2)  # this one.

            # query groups initialization, prompt as instance query.
        ...
        hs_pose, refpoint_pose = self.decoder(
                tgt=tgt_pose,  # **** torch.Size([b, q, 1+17, c]) ****
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            pose_query_sine_embed = gen_sineembed_for_position(refpoint_pose_reshape, self.d_model).reshape(nq, bs, np // 2, self.d_model)
