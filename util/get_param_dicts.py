...
def get_param_dict(args, model_without_ddp: nn.Module):
    ...
    if param_dict_type == 'default':
        param_dicts = [
            {"params": [p for n, p in model_without_ddp.named_parameters() if "backbone" not in n and 'adjacent_matrix' not in n and p.requires_grad]},
            {
                "params": [p for n, p in model_without_ddp.named_parameters() if "backbone" in n and p.requires_grad],
                "lr": args.lr_backbone,
            },
            {
                "params": [p for n, p in model_without_ddp.named_parameters() if "adjacent_matrix" in n and p.requires_grad],
                "lr": args.lr_adjacent_matrix,
            },
        ]
        return param_dicts
        ...
        param_dicts = [
            {
                "params":
                    [p for n, p in model_without_ddp.named_parameters()
                        ...
                        and not match_name_keywords(n, args.lr_text_encoder_names) and p.requires_grad],
                "lr": args.lr,
            },
            {
